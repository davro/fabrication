...
class FabricationEngineHtml4Test extends \PHPUnit_Framework_TestCase {
    ...
    public function testSanityLoadHtmlFile() {
        ...
        $this->assertFalse($this->engine->run(dirname(__FILE__).'/fixture/missing.html', 'file'));
        $this->assertTrue($this->engine->run(dirname(__FILE__).'/fixture/design.html', 'file'));
        $NodeList=$this->engine->getHtml();
        $this->assertInstanceOf('DOMNodeList', $NodeList);
        $this->assertEquals(1, $NodeList->length);
        $this->assertEquals('html', $NodeList->item(0)->tagName);

        // strange stripping of \n chars.
        $this->assertEquals(
            $this->engine->getDoctype().
            '<html>'.
            '<head></head>'.
            ...
            '<div id="content"></div>'."\n".
            '<div id="footer">'."\n".
            '<p>Generated by the <b>Fabrication</b> engine.</p>'."\n".
            '<p>&nbsp;</p>'."\n".
            '<p></p>'."\n".
    ...
    public function testSanityCreateProcessingInstruction() {
        $actual='<html><head><body></html>';
        $expected=$this->engine->getDoctype().'<html><head></head><body><?php echo PHP_VERSION; ?></body></html>';
        $this->assertTrue($this->engine->run($actual, 'string'));

        $this->engine->getElementsByTagName('body')->item(0)->appendChild(
            $this->engine->createProcessingInstruction('php', 'echo PHP_VERSION; ?')
        );

        $this->assertEquals($expected, $NodeList=$this->engine->outputHTML());
    }
